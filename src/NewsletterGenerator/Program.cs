...
do
{
    await PrintCopilotStartupStatusAsync();

    var selectedNewsletter = ResolveNewsletterType(newsletterArg) ?? PromptForNewsletterType();
    var selectedModel = await SelectModelAsync(modelArg);
    ...
    if (selectedNewsletter == NewsletterType.VSCode)
    {
        ...
        content = await AnsiConsole.Status()
...
static (bool ClearCache, bool ForceRefresh) PromptForStartupOptions(bool clearCacheFromArg, bool forceRefreshFromArg)
{
    ...
    var startupChoices = AnsiConsole.Prompt(
        new MultiSelectionPrompt<string>()
            ...
            .NotRequired()
            .AddChoices([
                "Use cache for this run",
                "Clear cache before run",
                "Force refresh (ignore cache reads this run)"
            ]));

    var useCacheSelected = startupChoices.Contains("Use cache for this run");
    var forceRefreshSelected = startupChoices.Contains("Force refresh (ignore cache reads this run)");

    var forceRefresh = forceRefreshFromArg || forceRefreshSelected;
    if (!forceRefreshFromArg && useCacheSelected)
        forceRefresh = false;

    return (
        ClearCache: clearCacheFromArg || startupChoices.Contains("Clear cache before run"),
...
static async Task PrintCopilotStartupStatusAsync()
{
    var cliPath = TryFindCopilotCliOnPath() ?? "copilot";

    string versionStatus;
    if (TryRunProcess(cliPath, "--version", out var versionStdOut, out var versionStdErr, out var versionExitCode) && versionExitCode == 0)
        versionStatus = string.IsNullOrWhiteSpace(versionStdOut) ? "Available" : versionStdOut;
    else
        versionStatus = string.IsNullOrWhiteSpace(versionStdErr) ? "Unavailable" : versionStdErr;
...
static async Task<string> SelectModelAsync(string? modelArg)
{
